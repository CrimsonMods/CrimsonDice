...
internal class PlayerService
{
    ...
    {
        var clans = GetClans().Where(x => x.ReadBuffer<SyncToUserBuffer>().Length > 0).ToList();
    ...
    public static Entity GetClanByGUID(Il2CppSystem.Guid clanGuid)
    {
        Entity clanEntity = GetClans().FirstOrDefault(entity => entity.Read<ClanTeam>().ClanGuid == clanGuid);
        return clanEntity != Entity.Null ? clanEntity : Entity.Null;
    }
}